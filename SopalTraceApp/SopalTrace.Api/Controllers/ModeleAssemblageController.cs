...
[Route("api/modeles-assemblage")]
[ApiController]
public class ModeleAssemblageController : ControllerBase
{
    private readonly IModeleAssemblageService _modeleAssService;

    public ModeleAssemblageController(IModeleAssemblageService modeleAssService)
    {
        _modeleAssService = modeleAssService;
    }
    ...
    {
        var data = await _modeleAssService.GetModelesByFiltersAsync(typeRobinet, natureComposant, operation, poste, familleProduit);
    ...
    {
        await _modeleAssService.UpdateModeleBrouillonAsync(id, request);
    ...
    {
        await _modeleAssService.ActiverModeleAsync(id, "SYSTEM");
    ...
    public async Task<IActionResult> CreerModele([FromBody] CreateModeleRequestDto request)
    {
        try
        {
            var id = await _modeleAssService.CreerModeleAsync(request);
            var data = await _modeleAssService.GetModeleByIdAsync(id);
    ...
    {
        var resultAss = await _modeleAssService.SupprimerBrouillonAsync(id);
        if (resultAss) return Ok(new { success = true, message = "Brouillon supprimé." });
    ...
    {
        var id = await _modeleAssService.CreerNouvelleVersionModeleAsync(request);
        var data = await _modeleAssService.GetModeleByIdAsync(id);
    ...
    public async Task<IActionResult> RestaurerModele([FromBody] RestaurerModeleRequestDto request)
    {
        try
        {
            var id = await _modeleAssService.RestaurerModeleArchiveAsync(request);
            var data = await _modeleAssService.GetModeleByIdAsync(id);
