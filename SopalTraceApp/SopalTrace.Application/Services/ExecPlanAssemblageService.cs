...
namespace SopalTrace.Application.Services
{
    public class ExecPlanAssemblageService : IExecPlanAssemblageService
    {
        ...
        public ExecPlanAssemblageService(IExecPlanAssemblageRepository repository, IExecutionOfService executionOfService)
        {
        ...
        public async Task<ExecPlanAssemblageDto> GetOrInitExecutionAsync(Guid execControleOfId, string? posteCode)
        {
            ...
            var (planAss, docRes) = await _repository.GetDocumentsAssemblageEtResultatAsync(codeArticle, posteCode);

            var statut = await _repository.GetStatutPlanAssAsync(execControleOfId, posteCode);
            ...
            // Filtrer par posteCode pour éviter de récupérer le statut d'un autre poste
            var statutResExisting = statutsAll
                .Where(s => s.TypeDocument == "RESULTAT_CF" || s.TypeDocument == "RCCF" || s.TypeDocument == "CTRL_POSTE")
                .Where(s => string.IsNullOrEmpty(posteCode) || s.PosteCode == null || s.PosteCode == posteCode)
                .FirstOrDefault();
            ...
            if (statut == null)
            {
                statut = new ExecControleDocumentStatut
                {
                    ...
                    PosteCode = !string.IsNullOrEmpty(posteCode) ? posteCode : execOf.ExecControleOfPostes.FirstOrDefault()?.PosteCode,
                    DocId = plan.Id
                };
            ...
            {
                bool changed = false;
                if (statut.DocId == null) { statut.DocId = plan.Id; changed = true; }
                if (statut.PosteCode == null && !string.IsNullOrEmpty(posteCode)) { statut.PosteCode = posteCode; changed = true; }
                if (changed) await _repository.SaveChangesAsync();
            }

            if (docRes != null)
            {
                var statutRes = statutResExisting ?? await _repository.GetStatutResultatCfAsync(execControleOfId, docRes.Id, posteCode);
                if (statutRes == null)
                {
                ...
                {
                    bool changed = false;
                    if (statutRes.DocId == null) { statutRes.DocId = docRes.Id; changed = true; }
                    if (statutRes.PosteCode == null && !string.IsNullOrEmpty(posteCode)) { statutRes.PosteCode = posteCode; changed = true; }
                    if (changed) await _repository.SaveChangesAsync();
                }
            }

            // Supprimer les anciennes tranches ECH|... (migration vers OccurrenceService)
            var staticTranchesAll = tranches.Where(t => t.TrancheHoraire != null && t.TrancheHoraire.StartsWith("ECH|")).ToList();
            ...
            // L'auto-configuration a été retirée à la demande de l'utilisateur
            ...
            var configuredCodes = dto.PostesConfigures.Select(p => p.PosteCode).ToHashSet();
            foreach (var execPoste in execOf.ExecControleOfPostes)
            {
                if (!string.IsNullOrEmpty(execPoste.PosteCode) && !configuredCodes.Contains(execPoste.PosteCode))
                {
                    dto.PostesConfigures.Add(new PosteHoraireDto
                    {
                        PosteCode = execPoste.PosteCode,
                        HeureDebut = "06:00",
                        HeureFin = "14:00"
                    });
                    configuredCodes.Add(execPoste.PosteCode);
                }
            }
            ...
            foreach (var secDto in sections)
            {
                ...
                else if (secDto.TypeSection == "LOT_OF" || secDto.TypeSection == "LOT_POSTE")
                {
                    // Ces sections n'ont plus de tranches. On vérifie directement dans Exec_ControleLigne_Reponse
                    var secLigneIds = secDto.LignesPlan.Select(l => l.Id)
                        .Concat(secDto.LignesResultat.Select(l => l.Id))
                        .Where(id => id != Guid.Empty)
                        .Distinct().ToList();

                    if (secLigneIds.Any() && secLigneIds.All(id => reponsesLignes.Any(r => r.DocumentLigneId == id)))
                    {
                        secDto.Resultats.Add(new ExecPlanAssemblageRowDto
                        {
                            Id = Guid.Empty,
                            Frequence = secDto.TypeSection == "LOT_POSTE" ? "Début de poste" : "Début OF",
                            Resultat = "C",
                            StatutNotif = "FAIT"
                        });
                    }
        ...
        public async Task<object> VerifierDocumentsAsync(Guid execControleOfId, string? posteCode)
        {
            ...
            var codeArticle = execOf.NumeroOfNavigation?.CodeArticle ?? "";
            var (planAss, docResultat) = await _repository.GetDocumentsAssemblageEtResultatAsync(codeArticle, posteCode);

            var statutPlan = await _repository.GetStatutPlanAssAsync(execControleOfId, posteCode);
        ...
        public async Task<ExecPlanAssemblageDto> ConfigurerHorairesAsync(Guid execControleOfId, ConfigHeuresPosteRequest request)
        {
            ...
            foreach (var poste in request.Postes)
            {
                ...
                foreach (var sec in plan.DocumentSections)
                {
                    ...
                    // Aucune tranche n'est générée pour LOT_POSTE ou LOT_OF
                    // L'utilisateur souhaite voir uniquement la section globale (comme image2) sans découpage par tranches
                    // Les sections par défaut ne génèrent aucune tranche horaire (retiré)
                }
            }
        ...
        public async Task<bool> SaveResultatAsync(Guid execControleOfId, SaveResultatAssRequest request)
        {
            ...
            if (request.LignesControle != null && request.LignesControle.Any())
            {
                ...
                if (operateurId != Guid.Empty)
                {
                    ...
                    if (!string.IsNullOrEmpty(request.Contexte))
                    {
                        contexte = request.Contexte;
                    }
                    else if (!isReglageOnly)
                    {
                    ...
                    foreach (var l in request.LignesControle)
                    {
                        ...
                        {
                            var existing = await _repository.GetLigneReponseByLigneIdAsync(execControleOfId, l.LigneId, contexte);
                            if (existing != null)
                            {
            ...
            // Mettre à jour EstDemarrageTermine ou EstTermine pour PLAN_ASS et RCCF
            var statuts = await _repository.GetStatutsAssemblageAsync(execControleOfId);
            foreach (var s in statuts)
            {
                if (request.Periodicite == "FIN_POSTE")
                {
                    s.EstTermine = true;
                }
                else
        ...
        public async Task<bool> CloturerExecutionAsync(Guid execControleOfId)
        {
            ...
            await _executionOfService.CloturerOfAsync(execControleOfId);
        ...
        {
            // === ÉTAPE 1 : TypeSection.Code — source de vérité pour les cas non-ambigus ===
            string tsCode = (sec.TypeSection?.Code ?? "").ToUpper();

            if (tsCode == "REGLAGE") return "REGLAGE";
            if (tsCode == "REGLAGE_PROD") return "REGLAGE_PROD";
            if (tsCode == "LOT_POSTE") return "LOT_POSTE";
            if (tsCode == "LOT_OF" || tsCode == "LOT") return "LOT_OF";
            if (tsCode == "EN_COURS" || tsCode == "ECHANT_NQA") return "ECHANTILLONNAGE";

            // === ÉTAPE 2 : Pour TypeSection = 'OF' ou absent → PerimetreControle de RegleEchantillonnage ===
            string? perimetre = sec.RegleEchantillonnage?.PerimetreControle?.ToUpper();
            if (perimetre == "POSTE") return "LOT_POSTE";
            if (perimetre == "OF") return "LOT_OF";
            if (perimetre == "ECHANTILLONNAGE") return "ECHANTILLONNAGE";

            // === ÉTAPE 3 : Fallback sur Code règle (compatibilité ascendante si PerimetreControle non renseigné) ===
            string regleCode = (sec.RegleEchantillonnage?.Code ?? "").ToUpper();
            if (regleCode == "REG-8B29P" || regleCode == "REG-43BD2") return "LOT_POSTE";
            if (regleCode == "REG-7A57N") return "LOT_OF";
            if (regleCode == "REG-9F43B" || regleCode == "REG-7A57C") return "ECHANTILLONNAGE";

            // === ÉTAPE 4 : Fallbacks textuels sur le Libellé (TRÈS FRAGILE - Gardé uniquement pour rétrocompatibilité) ===
            string lib = (sec.LibelleSection ?? "").ToLower();
            if (lib.Contains("réglage") && !lib.Contains("produc")) return "REGLAGE";

            // === Défaut : c'est de l'échantillonnage ===
