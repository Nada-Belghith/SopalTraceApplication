  ...
  <div class="w-full min-w-max">
    ...
    <table class="w-full min-w-[1400px] text-left border-collapse text-sm border-x border-b border-slate-300 rounded-b-lg shadow-sm">
      ...
      <thead class="bg-slate-900 text-white text-[11px] uppercase tracking-wider font-bold border-b border-slate-700 text-center">
          ...
          <template v-if="store.entete.afficheMoyenDetectionRisques">
            <th v-for="cCol in getCustomColumnsAfter('conformite', 'moyen_detection')" :key="cCol.key" :rowspan="hasSubHeaders ? 2 : 1" class="p-2 border-r border-slate-700 w-[10%] text-[10px] text-amber-400 uppercase">{{ cCol.label }}</th>
          </template>
      ...
      <template v-for="(ligne, lIdx) in store.lignesConformite" :key="ligne._uid">
        ...
        <template v-for="(group, gIdx) in ligne.groups" :key="group._uid">
          <tr v-for="(row, rIdx) in group.rows" :key="row._uid" 
            ...
            <template v-if="gIdx === 0 && rIdx === 0">
              <td v-for="cCol in getCustomColumnsAfter('conformite', 'risque')" :key="cCol.key" :rowspan="getLigneTotalRows(ligne)" class="p-2 border-r border-slate-400 align-top bg-amber-50/20">
            ...
            <template v-if="gIdx === 0 && rIdx === 0">
              <td v-for="cCol in getCustomColumnsAfter('conformite', 'methode')" :key="cCol.key" :rowspan="getLigneTotalRows(ligne)" class="p-2 border-r border-slate-400 align-top bg-amber-50/20">
                <textarea v-if="!props.isReadOnly" v-model="ensureColonnes(ligne)[cCol.key]" class="w-full text-xs font-bold text-slate-800 border border-slate-200 focus:border-amber-400 outline-none rounded p-1 resize-none bg-white/50" rows="2"></textarea>
                <div v-else class="text-xs font-bold text-slate-800">{{ ensureColonnes(ligne)[cCol.key] || '--' }}</div>
              </td>
            </template>
            ...
            <template v-if="rIdx === 0">
              <td v-for="cCol in getCustomColumnsAfter('conformite', 'periodicite')" :key="cCol.key" :rowspan="group.rows.length" class="p-2 border-r border-slate-400 align-top bg-amber-50/20">
                <textarea v-if="!props.isReadOnly" v-model="ensureColonnes(ligne)[cCol.key]" class="w-full text-xs font-bold text-slate-800 border border-slate-200 focus:border-amber-400 outline-none rounded p-1 resize-none bg-white/50" rows="2"></textarea>
                <div v-else class="text-xs font-bold text-slate-800">{{ ensureColonnes(ligne)[cCol.key] || '--' }}</div>
              </td>
            </template>
            ...
            <template v-if="store.entete.afficheMoyenDetectionRisques">
              <td v-for="cCol in getCustomColumnsAfter('conformite', 'moyen_detection')" :key="cCol.key" class="p-2 border-r border-slate-400 align-top bg-amber-50/20">
                <textarea v-if="!props.isReadOnly" v-model="ensureColonnes(ligne)[cCol.key]" class="w-full text-xs font-bold text-slate-800 border border-slate-200 focus:border-amber-400 outline-none rounded p-1 resize-none bg-white/50" rows="2"></textarea>
                <div v-else class="text-xs font-bold text-slate-800">{{ ensureColonnes(ligne)[cCol.key] || '--' }}</div>
              </td>
            </template>
