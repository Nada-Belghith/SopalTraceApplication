<template>
  <div v-if="visible" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
    ...
    <div class="bg-white text-slate-800 w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <div class="w-full">
          ...
          <div class="bg-[#0f172a] text-slate-100 rounded-xl p-6 shadow-lg border border-slate-800">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <!-- COLONNES ACTIVES -->
              <div class="space-y-4">
                <h3 class="text-xs font-bold text-slate-300 uppercase tracking-wider">Colonnes actives ({{ customOnlyColumns.length }})</h3>
                <div class="bg-[#1e293b]/50 border border-slate-700/50 rounded-xl h-[240px] p-4 flex flex-col gap-2 overflow-y-auto">
                  <div v-if="customOnlyColumns.length === 0" class="m-auto text-center text-slate-500 text-xs flex flex-col items-center gap-2">
                  ...
                  <div v-else v-for="(col, index) in customOnlyColumns" :key="col.key || index" class="bg-[#1e293b] border border-slate-700 rounded-lg p-3 flex justify-between items-center text-sm shadow-sm transition-all" :class="{'border-amber-500/50 shadow-amber-500/10': col.key === 'draft_col'}">
                    <div>
              ...
              <!-- EXTRA CONFIGURATION SLOT -->
              <slot name="extra-configuration"></slot>
            </div>
          </div>
        </div>
      </div>
                ...
                <tbody class="text-slate-600 bg-white border border-t-0 border-slate-200">
                  <tr class="hover:bg-slate-50">
                    <td v-for="col in previewColumns" :key="col.key" class="px-4 py-4 border-r border-slate-100 last:border-0">
                      ...
                      <span v-else-if="col.key === 'observations' || col.key === 'observation'" class="text-slate-400 border border-slate-200 rounded px-3 py-1.5 w-full inline-block bg-slate-50">Obs...</span>
...
const customOnlyColumns = computed(() => {
  let cols = [...customColumns.value]
  if (newCol.value.label.trim()) {
    cols.push({
      key: 'draft_col',
      label: newCol.value.label.trim(),
