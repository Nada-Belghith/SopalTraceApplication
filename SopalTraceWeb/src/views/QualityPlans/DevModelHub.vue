...
const filteredPlans = computed(() => {
  return modeles.value.filter(plan => {
    ...
    const designation = plan.designation ? plan.designation.toLowerCase() : '';
    const codeReference = plan.codeReference ? plan.codeReference.toLowerCase() : '';
    const codeRefStr = `${plan.nature || ''} ${plan.type || ''} ${codeReference}`.toLowerCase();
    const matchSearch = q === '' || libelle.includes(q) || designation.includes(q) || codeRefStr.includes(q);
